...
class Perceptron():
    ...
    def get_toy_model(self, transverse_field_coefficient):

        obs = [qml.PauliZ(i) @ qml.PauliZ(self.n_qubits-1) for i in range((self.n_qubits-1))]
        coeffs = [1.0 for i in range((self.n_qubits-1))]
        z_term = qml.Hamiltonian(coeffs, obs)
        ...
        obs = [qml.PauliY(i) for i in range((self.n_qubits))]
        coeffs = [transverse_field_coefficient for i in range((self.n_qubits))]
        transverse_field_term += qml.Hamiltonian(coeffs, obs)


        toy_hamiltonian = transverse_field_term + z_term
        ...
        energies = qml.eigvals(toy_hamiltonian, k=n_eigenvalues, which='SR')

        return toy_hamiltonian, energies
...
class NativePerceptron():
    ...
    def get_1d_ising_hamiltonian(self, transverse_field_coefficient):
        ...
        obs += [qml.PauliX(i) for i in range((self.n_qubits))]
        coeffs = [1.0 for i in range((self.n_qubits-1))]
        coeffs+= [transverse_field_coefficient]*self.n_qubits


        ising_hamiltonian = qml.Hamiltonian(coeffs, obs)
