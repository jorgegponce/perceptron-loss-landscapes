...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec 21 20:27:26 2023
...
def ghz(nqubits):
    l = np.array([1/np.sqrt(2)]+[0]*(2**(nqubits-1)-2)+[1/np.sqrt(2)])
    return qml.Hermitian(np.kron(np.outer(l,l),zero), wires=range(nqubits))
...
for rep in range(Nreps):
    ...
    print('Random parameters give initial accuracy: ', Init_loss)
    ...
    for i in range(n_explore):
        ...
        y = x + xi*np.random.uniform(0,1,size=x.size)
    ...
    print("Best accuracy achieved during exploration: ", best_loss)
...
for r in range(Nreps):
    axes[r,0].plot(range(start,n_epochs+1), curves_cold_start[r,start:], linewidth=3)
    axes[r,0].plot(range(start,n_epochs+1), curves_warm_start[r,start:], linewidth=3)
