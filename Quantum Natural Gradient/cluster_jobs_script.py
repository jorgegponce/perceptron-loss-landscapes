...
filename_template = 'perceptron_${QUBITS}_qubits_${PULSES}_pulses_${LEARNING_RATE}_lr.sh'

# Loop over each LEARNING_RATE value
for lr in learning_rates:
    # Prepare the filename with the current LEARNING_RATE
    filename = filename_template.format(qubits=QUBITS, pulses=BASIS, lr=lr)
    ...
    sbatch_command = f'sbatch perceptron_{QUBITS}_qubits_{BASIS}_pulses_{lr}_lr.sh'
